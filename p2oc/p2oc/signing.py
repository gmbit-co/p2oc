...
def create_change_only_psbt(fee_amount, lnd):
    ...
    output amount) will be of the given fee + the transaction fee to publish this PSBT.
    """
    # Use a dummy address to extract "funding" UTXO and change addresses
    dummy_address = create_dummy_p2wpkh_address()
    tx_template = walletmsg.TxTemplate(outputs={str(dummy_address): fee_amount})
    ...
    if psbt.get_fee() != original_fee + fee_amount:
        raise RuntimeError(
            f"Change-only PSBT does not have a fee ({psbt.get_fee()}) that matches the "
            + f"given fee amount={fee_amount} + the transaction fee={original_fee}."
        )
